...
def run_transaction_tests():
    ...
    import sqlite3
    conn = sqlite3.connect("/root/verv-dashboard/verv_os.db", timeout=5)
    pragmas = {}
    ...
    write_conn = sqlite3.connect("/root/verv-dashboard/verv_os.db", timeout=1)
    write_conn.execute("BEGIN IMMEDIATE")
    write_conn.execute("UPDATE properties SET notes = 'LOCK_TEST' WHERE id = 1")
    
    start = time.time()
    try:
        read_conn = sqlite3.connect("/root/verv-dashboard/verv_os.db", timeout=2)
    ...
    w_conn = sqlite3.connect("/root/verv-dashboard/verv_os.db", timeout=5)
    ...
    r_conn = sqlite3.connect("/root/verv-dashboard/verv_os.db", timeout=5)
